...
- 给当前 100 题评测集补上了题型标签，题型使用一组轻量但稳定的多标签分类：`事实查找 / 多跳推理 / 跨段聚合 / 带时间限制 / 容易混淆概念 / 需要拒答的问题`
- `scripts/build_eval_dataset.py` 生成 `data/evals/test_eval_set.json` 时会自动为每道题写入 `question_types`，因此后续你只维护 `data/raw/evaluatetest.md`，重建数据集后题型也会一起生成
- 评测运行与汇总链路同步支持题型维度：`run.json` 现在会保留每题的 `question_types`，`report.md` 会在每个 mode 下额外输出按题型聚合的 `correct / incorrect / insufficient` 统计
- 对旧 run 也做了向后兼容：如果老 `run.json` 没带 `question_types`，但还能找到对应的 dataset JSON，刷新报告时会自动回填题型统计，而不是统一落到“未分类”
- 评测与数据集测试已同步更新，锁住题型字段、100 题数据集分布和题型统计报告；全量测试当前为 `68 passed`

## v0.7.2

- 针对 100 题评测里暴露出来的共性问题，继续把 `/api/ask` 的生成护栏收成更通用的 validator，而不是再往 runtime 里加单题补丁：新增了“为什么/原则题不能只答成事实值”“降级题要先判断阈值方向再回答行为”“文档未明示时必须 fail-closed”“多例外列表不能被答成唯一例外”“已知优先条件范围外不要一律退成未明示”这几类校验
- `direct_read` 的文档内聚焦继续保持通用化：解释型问题现在会更优先参考同时命中“对象词 + 原因/例外信号”的 chunk，减少把标题锚点段误当成真正规则依据的情况；降级题也会优先参考阈值、候选事实和未确认标记相关段落
- 评测与 LM Studio 单元测试同步补到 100 题时代，锁住这批新的共性护栏，同时把默认评测集数量断言更新到 100；全量测试当前为 `67 passed`

## v0.7.1

- 继续沿着“去过拟合”的方向收紧 `/api/ask`：不再往主干里补更多题库级触发词，而是把生成护栏重新收成 3 组更泛化的约束：边界信息与结论一致、例外/限定条件与结论一致、多要点问题不能只答一半
- `direct_read` 的文档内聚焦逻辑也从少量题目级关键词加权，收敛成更通用的问题信号：会优先参考引号短语、英文术语、数字、分类词、步骤词、冲突词和边界词，再按原文顺序恢复后送给模型
- LM Studio 与评测测试继续保持绿灯，锁住这次泛化后的 validator 和 `direct_read` 聚焦行为；全量测试当前为 `60 passed`

## v0.7.0

- 对本地回答链路做了一次“去过拟合重构”：`lmstudio` 不再围绕 `24 小时外 / 420 / Query Mode / 跨分钟` 这类单题触发词堆规则，而是收敛成更泛化的题型识别与护栏，例如 `分类题 / 是非判断题 / 数值+条件题 / 数值+顺序题 / 边界题 / 冲突题`
- `direct_read` 的文档内轻量加权也从题库关键词切换成更通用的词项聚焦逻辑：优先看引号短语、英文术语、数字、选项词和流程词，而不是硬编码命中特定评测题的短语
- 生成校验层保留了对结构化输出、数值保真、边界不外推、冲突不强合并这些通用约束，但尽量改成依赖“问题形态 + 上下文证据”的泛化规则，而不是把单个 PDF 里的规则名直接写进主干逻辑
- LM Studio 与评测层测试同步调整，继续锁住这些通用能力；全量测试当前为 `60 passed`

## v0.6.3

- 继续沿着“少量 prompt、更多代码校验”的方向，新增 4 类更窄的生成校验：`Query Mode` 分类题必须直接落到 `Lookup/Explain`、上下文预算题必须同时回答 `1600 + 裁剪顺序 + 标题行限制`、跨分钟日志题必须明确写出“跨分钟不属于优先条件”、冲突处理题必须把“保留冲突 + 固定提示语 + 禁止强行合并”一起答全
- `direct_read` 现在在文档内轻量相关性排序之外，还会给少数稳定题型加一层很小的局部加权，例如 `Query Mode` 分类题会优先保留命中 `Lookup / Explain / Query Mode` 的 chunk，减少读全文时被事实性段落带偏
- 新增 LM Studio 与评测层单元测试，锁住 `test-md-015/016/024/031` 这批刚识别出的稳定残留错对应的校验与上下文选择行为

## v0.6.2

- `direct_read` 不再把整篇文档 chunk 全量粗喂给本地模型，而是先在目标文档内部做轻量相关性排序，只保留前 6 个更相关的 chunk，再按原文顺序恢复后送入回答阶段，减少小模型在长上下文里的噪声
- LM Studio 的纠偏现在最多允许 2 次窄范围重试；重试提示继续保持短，只强调“补齐缺失点，不要复述规则”
- 继续加硬生成校验：范围外问题不能把“未明示”写成“明确不适用”；命中短段例外项时，结论必须直接落到“不被强制合并”；冲突处理题缺固定提示语 / 保留冲突 / 禁止强行合并时会继续触发纠偏
- 新增评测与 LM Studio 单元测试，锁住 `direct_read` 的上下文裁剪，以及“第二次重试补齐冲突处理”这两类新行为
...
- 将 `AGENTS.md` 收紧为“精简宪法版”，只保留最硬的长期规则；具体执行细节继续下沉到 `DEVELOPMENT.md`
- 明确仓库的验证分工：`deterministic code` 继续走 TDD，`LLM behavior` 默认走 eval-driven development
