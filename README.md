...
- 对分类题、判定题、数值题、步骤题、边界题和冲突题，会追加少量题型级提示，减少答偏题型或只答半句
- 如果模型第一版答案没有直接命中题型、前后自相矛盾，或写出了上下文里没有的数字，系统会自动做一次轻量纠偏重试
- 这套纠偏更偏向代码校验，而不是继续堆很长的 prompt。它主要检查 3 类通用问题：边界说明和结论是否一致、例外/限定条件和结论是否一致、多要点问题有没有只答一半
- 继续往下的收紧也尽量保持“泛化”：现在还会额外识别 3 类高频共性问题，比如“为什么/原则题不能只答成具体事实”“降级题要先判断阈值方向”“文档未明示时不能先承认未说明、后面又继续猜”
- 数字校验会把 `3.3 / 4.3` 这类章节号从数字比较里排除，避免把目录编号误报成真正的数字错误；冲突处理题如果来源要求固定提示语，答案也必须把那句提示语完整带出来
- 后续如果继续扩展，不再鼓励把每一道失败题直接翻译成一条 runtime 规则。评测集仍然用来发现问题，但主干逻辑尽量只保留可泛化的回答约束
- 当前的纠偏最多允许 2 次窄范围重试，重点继续放在“补齐缺失点”，而不是继续拉长总 prompt
...
现在这份评测集还会自动带上 `question_types`，当前使用的多标签类别是:
...
- 保存整次运行的 `run.json`
- 生成一份初始 markdown 报告
- 同时把每道题的 `question_types` 一起写进结果，供后续按题型统计
...
- 已评审和未评审数量
- 错误类型分布
- 题型分布，以及每种题型下的 `correct / incorrect / insufficient`
- 一个很轻量的下一步建议
...
- 现在的 `direct_read` 不再默认把整份文档 chunk 全部塞给模型，而是先在目标文档里做一次轻量相关性筛选，只保留前 6 个更相关的 chunk，再按原文顺序恢复，减少小模型在长上下文里的噪声
- 这层筛选优先看问题里的通用信号：引号短语、英文术语、数字、分类词、步骤词、冲突词和边界词，而不是为某一份评测文档写死特定关键词
